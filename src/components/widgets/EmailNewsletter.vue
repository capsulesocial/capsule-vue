	...
	>
		...
		<div v-if="$store.state.session.id !== $route.params.id">
			...
			<p v-if="profile.name !== ``" class="text-gray5 dark:text-gray3 text-sm mb-4">
				Create email newsletters to be aware of {{ profile.name }}'s new posts directly in your inbox:
			</p>
			<p v-else class="text-gray5 dark:text-gray3 text-sm mb-4">
				Create email newsletters to be aware of @{{ profile.id }}'s new posts directly in your inbox:
			</p>
